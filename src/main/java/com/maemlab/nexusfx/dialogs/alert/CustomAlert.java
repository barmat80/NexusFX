...
public class CustomAlert {
	...
	private final ButtonType[] confirmationButtons;
	private javafx.scene.control.Alert alert;

	public CustomAlert(Stage ownerStage, AlertType alertType, String title, String header, String contentText, String exception, ButtonType[] confirmationButtons) {
	...
	public Optional<ButtonType> show() {
		...
		stage.getIcons().addAll(ownerStage.getIcons()); // Copy icons from owner stage
	...
	private void setConfirmationButtons() {
		if(confirmationButtons != null)
			alert.getButtonTypes().setAll(confirmationButtons);
	}
